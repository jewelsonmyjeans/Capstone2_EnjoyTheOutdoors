...
const searchTermSelect = document.querySelector("#search-term");
...
searchTypeSelect.addEventListener("change", () => {
  searchTermSelect.innerHTML = '<option value="">Select a search term</option>';

  if (searchTypeSelect.value === "location") {
    locationsArray.forEach((location) => {
      searchTermSelect.innerHTML += `<option value="${location}">${location}</option>`;
    });
  } else if (searchTypeSelect.value === "park-type") {
    parkTypesArray.forEach((parkType) => {
      searchTermSelect.innerHTML += `<option value="${parkType}">${parkType}</option>`;
    });
  }
});
...
searchButton.addEventListener("click", () => {
  ...
  if (results.length === 0) {
    searchResultsDiv.textContent = "No results can be found";
  } else {
    results.forEach((park) => {
      searchResultsDiv.innerHTML += `
        ...
        park.State
                        ...
                        <p class="card-text">Phone: ${park.Phone || "N/A"}</p>
...
refreshButton.addEventListener("click", () => {
  searchTypeSelect.value = "nothing";
  searchTermSelect.innerHTML = '<option value="">Select a search term</option>';
  searchResultsDiv.innerHTML = "";
});
